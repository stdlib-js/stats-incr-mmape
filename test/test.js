...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrmmape, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrmmape( 3 ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the accumulator function computes a moving mean absolute percentage error incrementally', function test( t ) {
	...
	expected = [
		...
		(100.0/2.0)*( ((3.0-2.0)/3.0)+abs((1.0-3.0)/1.0) ),
		(100.0/3.0)*( ((3.0-2.0)/3.0)+abs((1.0-3.0)/1.0)+abs((2.0-5.0)/2.0) ),
		(100.0/3.0)*( abs((1.0-3.0)/1.0)+abs((2.0-5.0)/2.0)+((4.0-4.0)/4.0) ),
		(100.0/3.0)*( abs((2.0-5.0)/2.0)+((4.0-4.0)/4.0)+((10.0-3.0)/10.0) ),
...
tape( 'if not provided an input value, the accumulator function returns the current mean absolute percentage error', function test( t ) {
	...
	t.strictEqual( acc(), 65.0, 'returns expected value' );
	t.end();
});
